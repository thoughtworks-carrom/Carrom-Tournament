-- Remove team "Srinu Lakku / Bolem Poorna Rama Satya Chandu"
...
WHERE c.name = 'Men''s Doubles'
  ...
  AND (
    (
      lower(trim(p1.name)) LIKE '%srinu%lakku%'
      AND lower(trim(p2.name)) LIKE '%bolem%'
    )
    OR (
      lower(trim(p2.name)) LIKE '%srinu%lakku%'
      AND lower(trim(p1.name)) LIKE '%bolem%'
    )
  );
...
BEGIN
  ...
  FROM players
  WHERE lower(trim(name)) LIKE '%srinu%lakku%'
  LIMIT 1;

  SELECT id INTO v_p2_id
  FROM players
  WHERE employee_id = '45062'
     OR lower(trim(name)) LIKE '%bolem poorna%'
  LIMIT 1;

  IF v_p1_id IS NULL THEN
    RAISE EXCEPTION 'Player not found: Srinu Lakku (check name in players table)';
  END IF;

  IF v_p2_id IS NULL THEN
    RAISE EXCEPTION 'Player not found: Bolem Poorna Rama Satya Chandu (45062)';
  END IF;
  ...
  RAISE NOTICE 'Removed Srinu Lakku / Bolem from Men''s Doubles Group B';
END;
$$;
